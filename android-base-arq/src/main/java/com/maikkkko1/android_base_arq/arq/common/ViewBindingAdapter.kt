...
@BindingAdapter("showOnSuccessOrErrorOrInitializing", requireAll = false)
fun View.showOnSuccessOrErrorOrInitializing(state: ComponentState<*>?) {
    state?.also {
        showViewByState(
            view = this,
            isToShow = state is ComponentState.Success || state is ComponentState.Error || state is ComponentState.Initializing,
            animate = false
        )
    }
}
...
private fun showViewByState(view: View, isToShow: Boolean, animate: Boolean, hideState: Int = View.GONE) {
